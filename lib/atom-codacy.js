			...
			if (this.readyState == 4 && this.status == 200) {
				...
				if (xhr.response.data.length)
				{
					...
					for (i in xhr.response.data) {
						...
						var _this = xhr.response.data[i],
							issueRow = document.createElement('div');
							
						issueRow.classList.add('issue', _this.level.toLowerCase());
						issueRow.setAttribute('data-line-number', "Line: " + _this.startLine);
						issueRow.setAttribute('data-cost', _this.cost);
						issueRow.innerText = _this.message;
						
						content.appendChild(issueRow);
						//console.log(_this);
					}
				}
